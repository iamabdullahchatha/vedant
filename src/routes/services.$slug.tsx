...
function ServiceDetailPage() {
  ...
  if (!service) {
    return (
      <SiteLayout>
        ...
        >
          <Link to="/services" className="inline-flex items-center gap-2 rounded-full bg-white px-6 py-3 text-sm font-semibold text-primary shadow-elegant">
  ...
  const related = getServicesByCategory(service.categorySlug).filter((s) => s.slug !== service.slug).slice(0, 4);
  ...
  return (
    <SiteLayout>
      <PageHero
        ...
        title={<>{service.name} — <span className="text-gradient-brand bg-clip-text">{service.tagline}</span></>}
      ...
      >
        <div className="flex flex-wrap gap-4">
          ...
          <Link to="/services" className="inline-flex items-center gap-2 rounded-full border border-white/25 bg-white/5 backdrop-blur px-6 py-3 text-sm font-semibold text-white hover:bg-white/10 transition">
            ...
            <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.5 }}>
              ...
              <div className="mt-8 grid grid-cols-1 sm:grid-cols-2 gap-4">
                {service.capabilities.map((c) => (
                  <div key={c} className="flex items-start gap-3 rounded-xl border border-border/60 bg-white p-4 shadow-card-soft">
          ...
          <div className="space-y-6">
            <div className="rounded-2xl border border-border/60 bg-gradient-soft p-6">
              ...
              <p className="mt-4 text-sm text-muted-foreground leading-relaxed">
                Part of our {category.name.toLowerCase()} practice — engineered by global delivery teams
